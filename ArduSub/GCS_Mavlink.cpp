...

{
    ...
    case MAVLINK_MSG_ID_PING: { // MAV ID: 4
        ...
        // printf("get mavlink data: gripper %d", packet.seq);        
        // set gripper pwm value
        sub.gripper_set(packet.seq);
        break;
    }

    // naodai: light
    case MAVLINK_MSG_ID_PARAM_VALUE: {   // MAV ID: 22
        // decode packet
        mavlink_param_value_t packet;
        mavlink_msg_param_value_decode(&msg, &packet);
        // printf("get mavlink data: light%d", packet.param_index);
        // set light pwm value
        sub.light_set(packet.param_index);
        break;
    }

    //naodai: motor
    case MAVLINK_MSG_ID_SET_ACTUATOR_CONTROL_TARGET: {   // MAV ID: 139
        // decode packet
        mavlink_set_actuator_control_target_t packet;
        mavlink_msg_set_actuator_control_target_decode(&msg, &packet);
        float motor_control[8];
        mavlink_msg_set_actuator_control_target_get_controls(&msg, motor_control);
        sub.motor_set(motor_control);
        break;

    }

    // naodai: control t265 opened and closed (pwm)
    // case MAVLINK_MSG_ID_COMMAND_INT: { // MAV ID: 75
        
        // decode packet
        // mavlink_command_int_t packet;
        // mavlink_msg_command_int_decode(&msg, &packet);
        // sub.t265_power_set(packet.param1);
        // break;
